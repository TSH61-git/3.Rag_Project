...
@dataclass
class ResponseSynthesizedEvent(Event):
    response: str
    confidence_score: float
...
class RAGWorkflow(Workflow):
    ...
    async def validate_input(self, ctx: Context, ev: StartEvent) -> InputValidatedEvent | StopEvent:
        ...
        # Initialize state
        await ctx.set("total_attempts", 0)
        await ctx.set("all_nodes_seen", [])
    ...
    @step
    async def retrieve_nodes(self, ctx: Context, ev: InputValidatedEvent | NeedMoreContextEvent) -> NodesRetrievedEvent | StopEvent:
        """Step 2: Retrieve relevant nodes from index"""
        
        # Handle both event types
        if isinstance(ev, InputValidatedEvent):
            query = ev.query
            attempt = 1
            top_k = 3
            ...
            query = ev.query
            attempt = ev.attempt
            top_k = 5  # Get more nodes on retry
        ...
        # Update state
        await ctx.set("total_attempts", attempt)
        ...
        try:
            ...
            # Validation: check if nodes have content
            ...
            all_nodes = await ctx.get("all_nodes_seen", default=[])
            all_nodes.extend(valid_nodes)
            await ctx.set("all_nodes_seen", all_nodes)
            
            return NodesRetrievedEvent(query=query, nodes=valid_nodes)
    ...
    @step
    async def check_quality(self, ctx: Context, ev: NodesRetrievedEvent) -> QualityCheckedEvent | NeedMoreContextEvent | StopEvent:
        """Step 3: Check quality and route based on confidence"""
        nodes = ev.nodes
        query = ev.query
        ...
        attempt = await ctx.get("total_attempts", default=1)
        
        # Quality thresholds
        GOOD_THRESHOLD = 0.3
        MIN_THRESHOLD = 0.15
        
        # Routing logic based on quality
        if avg_score >= GOOD_THRESHOLD:
            ...
            # Very low quality - try again if possible
    ...
    @step
    async def synthesize_response(self, ctx: Context, ev: QualityCheckedEvent) -> ResponseSynthesizedEvent | StopEvent:
...
Provide a clear, concise answer based on the documentation above. If the documentation doesn't contain enough information, say so."""
        ...
        # Call LLM using Cohere Chat API
        try:
    ...
    @step
    async def format_response(self, ctx: Context, ev: ResponseSynthesizedEvent) -> StopEvent:
        """Step 5: Format final response with metadata"""
        print(f"[Step 5] Formatting final response")
        
        # Get state information
        total_attempts = await ctx.get("total_attempts", default=1)
        all_nodes = await ctx.get("all_nodes_seen", default=[])
        
        response = ev.response
        confidence = ev.confidence_score
        
        # Add metadata footer if multiple attempts or low confidence
        if total_attempts > 1:
